...
exports.Y = exports.Yarra = void 0;
class Yarra extends Array {
    ...
    static toIndices(input, shape) {
        ...
        const spacingOperators = {
            ...
            "#": (start, end, step) => Yarra.range({ start, end, step, inclusive: true }),
            ">": (start, n, step) => Yarra.range({ start, end: start + n, step }),
        };
        ...
        for (let [dimension, i] of dimensions.full) {
            ...
            for (let part of dimension.split(/[ ,]/)) {
                ...
                for (let op in spacingOperators) {
                    ...
                    if (inputs.length === 0) {
                        if (op === ":") {
                            dimensionIndices.push(...spacingOperators[op](0, shape[i]));
                        }
                        else
                            return;
                    }
                        ...
                        if (part.startsWith(op)) {
                            dimensionIndices.push(...spacingOperators[op](0, a));
                        }
                        else {
                            if (op === ":")
                                dimensionIndices.push(...spacingOperators[op](a, shape[i]));
                            else
                                return;
                        }
                    }
    ...
    static range({ start = 0, end, step = 1, inclusive = false, }) {
        ...
        let n = Math.ceil(steps) + (inclusive && Number.isInteger(steps) ? 1 : 0);
        if (n <= 0)
            return new Yarra();
        let arr = new Yarra(n);
        for (let i = 0; i < n; i++)
    ...
    static rangeMap({ start = 0, end, step = 1, inclusive = false, f = (x) => x, }) {
        // inclusive is for ending
        let steps = (end - start) / step;
        let n = Math.ceil(steps) + (inclusive && Number.isInteger(steps) ? 1 : 0);
        if (n <= 0)
        ...
        let arr = new Yarra(n);
        for (let i = 0; i < n; i++)
            arr[i] = f(start + i * step);
        return arr;
    }
    static linSpace(start = 0, end = 1, n = 2) {
        if (!Number.isInteger(n))
            throw Error("n not an integer");
        if (n <= 1)
    ...
    set(...indices) {
        ...
        return (x) => {
            let output = this;
            for (let i of indices.slice(0, -1))
                output = output[i];
        ...
        const setFromRunner = (arr, template, values, location) => {
            ...
            for (let [x, i] of template.full) {
                ...
                if (x[0] instanceof Array)
                    ...
                    curr.set(...x)(values.get(...location, i));
            }
            return this;
        };
    ...
    deepMap(f) {
        let deepMapRunner = (arr, f, indices = []) => {
            arr = new Yarra(arr);
            if (arr[0] instanceof Array)
                return arr.map((x, i) => deepMapRunner(x, f, [...indices, i]));
            else
                return arr.map((x, i) => f(x, [...indices, i]));
        };
    ...
    takeWhile(f) {
        for (let i = 0; i < this.length; i++)
            if (!f(this[i], i))
                return new Yarra(this.slice(0, i));
        return this.clone();
    }
    takeWhileRight(f) {
        for (let i = this.length - 1; i >= 0; i--)
            if (!f(this[i], i))
                return new Yarra(this.slice(i));
        return this.clone();
    }
    dropWhile(f) {
        for (let i = 0; i < this.length; i++)
...
exports.Y = Y;
let test = Yarra.allocate(3, 3, 3);
console.dir(test.deepMap((x, i) => i), { depth: null });
//# sourceMappingURL=Yarra.js.map